...
struct Event
{
  ...
  std::vector<std::vector<Double_t>> waveform; // [ch][nsample]
  std::vector<Double_t>              max_adc;
  std::vector<Double_t>              integral;
  std::vector<Double_t>              integral_ped;
  std::vector<std::vector<Double_t>> leading;  // [ch][depth]
  std::vector<std::vector<Double_t>> trailing; // [ch][depth]
...
ProcessingNormal()
{
  ...
  event.unixtime = gUnpacker.get_node_header(NodeIdEB, hddaq::unpacker::DAQNode::k_unix_time);
  
  std::time_t unix_time = event.unixtime;
  std::tm* time_info = std::localtime(&unix_time);

  char buffer[80];
  std::strftime(buffer, sizeof(buffer), "%Y-%m-%d %H:%M:%S", time_info);
  std::cout << "Event " << event.evnum << " : " << buffer << std::endl;
  ...
  // HF1(1, 0);
    ...
    // --- 1) PlaneId を連番に変換するマッピングを作成 ---
    std::map<int,int> planeMap;  // rawPlaneId → localPlaneId
    int nextPlane = 0;
    for (auto* hit : cont) {
        if (!hit) continue;
        int rawPlane = hit->PlaneId();
        if (planeMap.find(rawPlane) == planeMap.end()) {
            planeMap[rawPlane] = nextPlane++;
        }
    }

    // --- 2) グローバルチャンネル番号の最大値を計算 ---
    int max_ch = 0;
    for (auto* hit : cont) {
        if (!hit) continue;
        int plid = planeMap[hit->PlaneId()];
        int ch = plid * 32 + hit->SegmentId();
        max_ch = std::max(max_ch, ch + 1);
    }

    // --- 3) 波形配列を初期化（安全のため assign で内部ベクトルも初期化） ---
    if (max_ch == 0) max_ch = 1;
    event.waveform.assign(max_ch, {});
    event.leading.assign(max_ch, {});
    event.trailing.assign(max_ch, {});
    ...
    for (int i = 0; i < (int)cont.size(); ++i) {
      ...
      // Int_t seg  = hit->SegmentId();
        ...
        // std::cout << "Plane: " << hit->PlaneId()
        //     << "  Segment: " << hit->SegmentId() << std::endl;

      // Int_t ch = plid*NumOfSegRayraw+seg; //plid=0,1,2,...

      Int_t cid   = hit->DetectorId();
      Int_t seg   = hit->SegmentId();
      Int_t plid  = planeMap[hit->PlaneId()]; // 連番化した plane
      Int_t ch    = plid * 32 + seg;

      Int_t plid_raw = hit->PlaneId(); // 元の plane id
      event.plane.push_back(plid_raw); // 元の plane id を保存
      ...
      Int_t hid_wf           = RAYRAWHid + (ch+1)*1000 + 0;  // Raw Waveform
      Int_t hid_adc          = RAYRAWHid + (ch+1)*1000 + 1;  // Max ADC
      Int_t hid_integral     = RAYRAWHid + (ch+1)*1000 + 2;  // Integral
      Int_t hid_integral_ped = RAYRAWHid + (ch+1)*1000 + 3;  // Pedestal Integral
      Int_t hid_tdc_l        = RAYRAWHid + (ch+1)*1000 + 4;  // TDC Leading
      Int_t hid_tdc_t        = RAYRAWHid + (ch+1)*1000 + 5;  // TDC Trailing
      Int_t hid_tdc_first    = RAYRAWHid + (ch+1)*1000 + 6;  // TDC First
      Int_t hid_dE           = RAYRAWHid + (ch+1)*1000 + 10; // dE(ADC)
      Int_t hid_dE_q         = RAYRAWHid + (ch+1)*1000 + 11; // dE(Integral)

      // TDC Leading
      for (const auto& tdc_l : hit->GetArrayTdcLeading() ) {
	// HF1(hid_tdc_l, tdc_l);
	event.leading[ch].push_back(tdc_l);
	if(tdc_l > tdc_first)
	  tdc_first = tdc_l;
	...
	// HF1(hid_tdc_t, tdc_t);
	event.trailing[ch].push_back(tdc_t);
	...
	event.waveform[ch].push_back(fadc);
	// Max ADC
