...
//_____________________________________________________________________________
TList*
HistMaker::createRAYRAW(Bool_t flag_ps)
{
  TString strDet = CONV_STRING(kRAYRAW);
  name_created_detectors_.push_back(strDet);
  if(flag_ps){
    name_ps_files_.push_back(strDet);
  }
...
#if SINGLE_PLANE
  { ///// Waveform
    TString strSubDir  = "Waveform";
    //    TString strSubDir  = CONV_STRING(kFADC);
    const char* nameSubDir = strSubDir.Data();
    TList *sub_dir = new TList;
    sub_dir->SetName(nameSubDir);
    Int_t target_id = getUniqueID(kRAYRAW, 0, kFADC, 0);
    for( Int_t i=0; i<NumOfSegRAYRAW; ++i ){
      const char* title = NULL;
      if( i<NumOfSegRAYRAW ){
	Int_t seg = i; // 0 origin
	title = Form("%s_%s_ch%d", nameDetector, nameSubDir, seg);
      }else{
	// Int_t seg = i-NumOfSegRAYRAW; // 0 origin
	// title = Form("%s_%s_ch%dD", nameDetector, nameSubDir, seg);
      }
      sub_dir->Add(createTH2(target_id +i, title, // 0 origin
			     500, 0, 500,
			     1024, 0, 1024,
			     "Sample #", "ADC [ch]"));
    }
  ...
  { ///// Max ADC
    ...
    TList *sub_dir = new TList;
    sub_dir->SetName(nameSubDir);
    Int_t target_id = getUniqueID(kRAYRAW, 0, kADC, 0);
    for(Int_t i=0; i<NumOfSegRAYRAW; ++i){
      const char* title = NULL;
      if(i < NumOfSegRAYRAW){
	Int_t seg = i; // 0 origin
	...
	// title = Form("%s_%s_ch%dD", nameDetector, nameSubDir, seg);
      ...
      sub_dir->Add(createTH1(target_id + i, title, // 0 origin
			     1024, 0, 1024,
			     "ADC [ch]", ""));
    }
  ...
  { ///// Integral
    TString strSubDir  = "Integral";
    //    TString strSubDir  = CONV_STRING(kQDC);
    const char* nameSubDir = strSubDir.Data();
    TList *sub_dir = new TList;
    sub_dir->SetName(nameSubDir);
    Int_t target_id = getUniqueID(kRAYRAW, 0, kQDC, 0);
    for(Int_t i=0; i<NumOfSegRAYRAW; ++i){
      const char* title = NULL;
      if(i < NumOfSegRAYRAW){
	Int_t seg = i; // 0 origin
  ...
  { ///// Waveform w/ Leading
    TString strSubDir  = "Waveform w Leading";
    //     TString strSubDir  = CONV_STRING(kFADCwTDC);
    const char* nameSubDir = strSubDir.Data();
    TList *sub_dir = new TList;
    sub_dir->SetName(nameSubDir);
    Int_t target_id = getUniqueID(kRAYRAW, 0, kFADCwTDC, 0);
    for(Int_t i=0; i<NumOfSegRAYRAW; ++i ){
      const char* title = NULL;
      if( i<NumOfSegRAYRAW ){
	Int_t seg = i; // 0 origin
  ...
  { ///// Waveform w/o Leading
    TString strSubDir  = "Waveform wo Leading";
    const char* nameSubDir = strSubDir.Data();
    TList *sub_dir = new TList;
    sub_dir->SetName(nameSubDir);
    Int_t target_id = getUniqueID(kRAYRAW, 0, kADCwTDC, 0);
    for(Int_t i=0; i<NumOfSegRAYRAW; ++i ){
      const char* title = NULL;
      if( i<NumOfSegRAYRAW ){
	Int_t seg = i; // 0 origin
  ...
  { ///// TDC Leading
    //    TString strSubDir  = CONV_STRING(kTDC);
    TString strSubDir  = "TDC Leading";
    const char* nameSubDir = strSubDir.Data();
    TList *sub_dir = new TList;
    sub_dir->SetName(nameSubDir);
    Int_t target_id = getUniqueID(kRAYRAW, 0, kTDC, 0);
    for(Int_t i=0; i<NumOfSegRAYRAW; ++i){
      const char* title = NULL;
      if(i < NumOfSegRAYRAW){
	Int_t seg = i; // 0 origin
	...
	// Int_t seg = i-NumOfSegBH1; // 0 origin
	// title = Form("%s_%s_ch%dD", nameDetector, nameSubDir, seg);
      }
      sub_dir->Add(createTH1(target_id + i, title, // 0 origin
			     //			     10000, 0, 400000,
  ...
  { ///// TDC Trailing (represents as TOT, temporarily)
    //    TString strSubDir  = CONV_STRING(kTOT);
    TString strSubDir  = "TDC Trailing";
    const char* nameSubDir = strSubDir.Data();
    TList *sub_dir = new TList;
    sub_dir->SetName(nameSubDir);
    Int_t target_id = getUniqueID(kRAYRAW, 0, kTOT, 0);
    for(Int_t i=0; i<NumOfSegRAYRAW; ++i){
      const char* title = NULL;
      if(i < NumOfSegRAYRAW){
	Int_t seg = i; // 0 origin
  ...
  { ///// Waveform
    ...
    sub_dir->SetName(nameSubDir);
    Int_t target_id = getUniqueID(kRAYRAW, 0, kFADC, 0);
    for (Int_t i=0; i<NumOfPlaneRAYRAW; ++i){
      for( Int_t j=0; j<NumOfSegRAYRAW; ++j ){
	const char* title = NULL;
	if( j<NumOfSegRAYRAW ){
	  Int_t plane = i; // 0 origin
	  Int_t seg = j; // 0 origin
	  title = Form("%s%d_%s_ch%d", nameDetector, plane+1, nameSubDir, seg);
	}else{
	  // Int_t seg = i-NumOfSegRAYRAW; // 0 origin
	  // title = Form("%s_%s_ch%dD", nameDetector, nameSubDir, seg);
	}
	sub_dir->Add(createTH2(target_id + i*NumOfSegRAYRAW + j, title, // 0 origin
			       500, 0, 500,
			       1024, 0, 1024,
			       "Sample #", "ADC [ch]"));
      }
  ...
  { ///// Max ADC
	...
	sub_dir->Add(createTH1(target_id + i*NumOfSegRAYRAW + j, title, // 0 origin
			       1024, 0, 1024,
			       "ADC [ch]", ""));
      }
  ...
  //     if( i<NumOfSegBH1 ){
  // 	Int_t seg = i; // 0 origin
  // 	title = Form("%s_%s_ch%dU", nameDetector, nameSubDir, seg);
  //     }else{
  // 	Int_t seg = i-NumOfSegBH1; // 0 origin
  // 	title = Form("%s_%s_ch%dD", nameDetector, nameSubDir, seg);
  //     }
