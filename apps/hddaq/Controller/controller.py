...
class Controller(Frame):
  ...
  def update_status_window(self):
    ...
    # Check trigger rate for 60 secs and raise alarm when the rate is zero
    current_evnum = int(status.dist_evnum)
    trigger_rate = ((current_evnum - self.prev_evnum)
                   / (time.time() - self.status_timestamp))
    
    tdiff = time.time() - self.status_timestamp
    logger.info(f"Trigger rate: {trigger_rate:3.2f} Hz, TDiff: {tdiff:3.2f} sec")

    if self.daq_state==StatusList.S_RUNNING and trigger_rate==0 and tdiff>60:
        logger.warning("Trigger rate zero for more than 60 sec!")
        os.system(sound_command)
    
    if self.prev_evnum != current_evnum and tdiff > 60:
        print("Updating evnum and trigger rate")
        self.prev_evnum = current_evnum
        self.status_timestamp = time.time()
